...
import com.zeng.monitor.core.CameraSource;
import com.zeng.monitor.core.SocketCamera;
...
public class MonitorView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private MonitorParameter cmPara;
	/**
	...
	public MonitorView(Context context, AttributeSet attrs) {
        ...
        thread = new MonitorThread(holder);
	...
	class MonitorThread extends Thread{
		...
		public MonitorThread(SurfaceHolder surfaceHolder) {
			super();
...
//		}
		...
		private boolean captureImage(String ip, int port, int width, int height){
	        ...
	        if (!cs.open()) { /* deal with failure to obtain camera */ 
	        	ActivityUtil.showAlert(MonitorView.this.getContext(), "Error", "不能连接远端服务器！", "确定");
	        	return false;
	        }
	        cs.capture(c); //capture the frame onto the canvas
		...
		public boolean saveImage(){
			...
			else if(cs.saveImage(cmPara.getLocal_dir(), now+".JPEG"))
	...
	public MonitorParameter getCmPara() {
		return cmPara;
	}

	public void setCmPara(MonitorParameter cmPara) {
		this.cmPara = cmPara;
	}
